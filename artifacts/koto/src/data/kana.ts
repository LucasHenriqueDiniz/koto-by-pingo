...
interface KanaCell {
  /** Unique id suffix (e.g. 'a', 'shi', 'kya'). Combined with the script prefix it becomes the final id. */
  id: string;
  hira: string;
  kata: string;
  romaji: string;
  ...
  column: string;
  group: KanaGroup;
  /** ids (suffix) of similar kana, within the same script. */
  similarTo?: string[];
  /** Example word (applied to hiragana only). */
  example?: KanaExample;
}
